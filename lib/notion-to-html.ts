            ...
            case 'p': {
              ...
              if (pageId && recordMap) {
                ...
                const pageTitle = refBlock?.properties?.title
                  ...
                  : 'Untitled'
                const pageUrl = `${config.host}/${pageId.replaceAll('-', '')}`
...
function mergeAdjacentLists(html: string): string {
  ...
  html = html.replaceAll(
    /(<!--UL--><li>.*?<\/li><!--\/UL-->(?:\s*<!--UL--><li>.*?<\/li><!--\/UL-->)*)/gs,
    (match) => {
      const items = match.replaceAll(/<!--\/?UL-->/g, '')
      return `<ul>${items}</ul>`
    }
  )
  // Replace adjacent OL markers with proper <ol> wrapper
  html = html.replaceAll(
    /(<!--OL--><li>.*?<\/li><!--\/OL-->(?:\s*<!--OL--><li>.*?<\/li><!--\/OL-->)*)/gs,
    (match) => {
      const items = match.replaceAll(/<!--\/?OL-->/g, '')
      return `<ol>${items}</ol>`
    }
  )
  return html
